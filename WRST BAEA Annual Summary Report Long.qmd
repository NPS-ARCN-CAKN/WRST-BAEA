...
Summary_SurveyDatesByYearDataFrame = dbGetQuery(Connection,Sql) # Get the data from the database into a data frame
...
Summary_SuccessByYearDataFrame = dbGetQuery(Connection,Sql) 
...
# AND (StatusCode_P IN ('S', 'IF')) -- Successful or faild, ignore not found and not counted records",sep='')
# 
# # Get data frame from query
# Dataset_ProductivityDataFrame = dbGetQuery(Connection,Sql)
...
Sql = paste("SELECT  [Year]
,[RiverSegment]
,RiverSegmentCode as [River Segment]
,[Nests located]
,[Occupied]
,[Successful]
,[Productivity unknown]
...
ORDER BY Year DESC,RiverSegmentCode",sep="")

# Get the data from the database into a data frame
SuccessByYearRiverSegmentDF = dbGetQuery(Connection,Sql) 
...
ggplot(data=Summary_SuccessByYearDataFrame) +
  ...
  geom_smooth(mapping = aes(x=Year,y=`% nest success`),linetype="solid",color="black",method = "lm", se = FALSE, na.rm = FALSE,level = .95) +
  ...
  # % nest success linear regression model with 95% SE shaded area
  geom_smooth(mapping = aes(x=Year,y=`% nest occupied`),linetype="solid",color="black",method = "lm", se = FALSE, na.rm = FALSE,level = .95) +
...
ggplot(data=Summary_SuccessByYearDataFrame) +
  ...
  geom_smooth(mapping = aes(x=Year,y=`Young/successful nest`),linetype="solid",color="black",method = "lm", se = FALSE, na.rm = FALSE,level = .95) +
  ...
  geom_smooth(mapping = aes(x=Year,y=`Young/occupied nest`),linetype="solid",color="black",method = "lm", se = FALSE, na.rm = FALSE,level = .95) +
...
Summary_SuccessByRiverSegmentDataFrame = dbGetQuery(Connection,"SELECT 
RiverSegment
, Earliest
, Latest
...
ggplot(data=subset(SuccessByYearRiverSegmentDF,RiverSegment != "Copper River")) +
  ...
  geom_smooth(mapping = aes(x=Year,y=`% nest success`),method = "lm",color='black', se = FALSE) +
   # Minimal theme
  theme_minimal() +
...
ggplot(data=subset(SuccessByYearRiverSegmentDF,RiverSegment != "Copper River")) +
  ...
  geom_smooth(mapping = aes(x=Year,y=`Young/successful nest`),method = "lm",color='black', se = FALSE) +
  ylim(0,2) +
...
kable(select(SuccessByYearRiverSegmentDF,-RiverSegment), caption = paste("Table ",TableCounter,". Long term success and productivity metrics by year and river segment in the Copper River bald eagles monitoring study area.",sep=""))
