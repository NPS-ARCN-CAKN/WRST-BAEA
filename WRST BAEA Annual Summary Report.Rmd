...
SurveyYear = 2009  # Set this to the report year
```
...
colnames(CurrentSummaryDF_t)[1] = "Date" # Change the wierd first column with no name that R stuck in to Result
...
Table `r TableCounter`. Nest productivity summary, `r SurveyYear` bald eagles survey.
...
Table `r TableCounter`. Long term productivity.
...
Figure `r FigureCounter`. Nest success by year: Entire study area (R\^2=`r R2`, p=`r p`).

### Long term productivity by river segment

Table `r TableCounter`. Long term productivity by year and river segment.
...
Figure `r FigureCounter`. Young per occupied nest by year, entire study area (R\^2=`r R2`, p=`r p`).
