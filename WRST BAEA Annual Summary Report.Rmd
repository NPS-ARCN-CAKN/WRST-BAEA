...
Rivers = terra::vect(paste(ShpPath,"Alaska_Rivers_1M.shp",sep=""))

# Get the rivers shapefile
ParksShp = terra::vect(paste(ShpPath,"/GCS/AKParks_reprojected.shp",sep=""))
...
plot(ParksShp,col="lightgreen", axes=TRUE, main=Title,ylim = c(60.5, 62.9),xlim = c(-146, -142))
plot(Rivers, add=TRUE, col="LightBlue")
text(x = -143.3, y = 61.7, "Wrangell-St. Elias 
     National Park and Preserve",cex=.7) 
plot(NestsSpatialPoints, pch = 19, col = "Red",add=TRUE,cex=0.6)
...
# Boxplot the nest success, look for outlier dots
boxplot(SuccessByYearDF$`% nest success`)
...
# Query out the successful nests
...
ggplot(data=ProductivityDataFrame) +
  ...
  #geom_density_2d_filled(data=SuccessFulNestsDataFrame,aes(x = Longitude, y = Latitude), alpha=0.5,contour_var = "density",show.legend = FALSE,xlim = c(-180, -100), ylim = c(55, 65)) +
  ...
  #stat_density2d(data=SuccessFulNestsDataFrame,aes(x = Longitude, y = Latitude), alpha=0.5,contour_var = "density",    show.legend = TRUE) +
  ...
  ylab('Latitude')+
  theme_minimal() +
  geom_text(aes(x = -144, y = 61.9, label = "Wrangell - St. Elias 
National Park and Preserve"), size = 3, color = "black",stat = "unique")
