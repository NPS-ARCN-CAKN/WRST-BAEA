...
```
Figure `r FigureCounter`. Dispersion of successful bald eagle nests in the Copper River Valley in in `r SurveyYear`.
...
# Map Title
title = paste("Productivity, ", SurveyYear)

# Query out just the successful nests
Yng1 = subset(ProductivityDataFrame,NumYng = 1)
Yng2 = subset(ProductivityDataFrame,NumYng >= 2)
...
ggplot() +
  ...
  # Productivity data
  geom_point(data=Yng1, aes(x = Longitude, y = Latitude,colour=NumYng),color="gold",size=2,legend=TRUE) +
  geom_point(data=Yng2, aes(x = Longitude, y = Latitude,colour=NumYng),color="darkgreen",size=3,legend=TRUE) +
...
#kable(ProductiveNestsDataFrame)

```

Figure `r FigureCounter`. Dispersion of bald eagle productivity in the Copper River Valley in in `r SurveyYear`. Green dots show nests producing two young, gold dots show nests producing one young.
