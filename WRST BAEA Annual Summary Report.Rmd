...
YngPerOccupiedNest = signif(CurrentSummaryDF$`Young/occupied nest`,2)
YngPerNest = signif(CurrentSummaryDF$`Young/successful nest`,2)
PctOccupied = signif(CurrentSummaryDF$`% nest occupied`,2)
PctSuccessful = signif(CurrentSummaryDF$`% nest success`,2)
...
#Output
TableCaption = paste("Table ",TableCounter,". Results summary, ",SurveyYear," bald eagles survey.",sep="")
kable(CurrentSummaryDF_char_DF, caption=TableCaption)

# Increment the table counter
TableCounter = TableCounter + 1
```
```{r echo=FALSE, label="Success map",fig.alt=paste("Figure ",FigureCounter,". Dispersion of successful bald eagle nests in the Copper River Valley in in ",SurveyYear,". Green areas show areas where nests were most successful at producing young. Mean occupancy rate was ",CurrentSummaryDF$`% nest occupied`,"% and mean success rate was ",CurrentSummaryDF$`% nest success`,"%.",sep=''),fig.cap=paste("Figure ",FigureCounter,". Dispersion of successful bald eagle nests in the Copper River Valley in in ",SurveyYear,". Green areas show areas where nests were most successful at producing young. Mean occupancy rate was ",CurrentSummaryDF$`% nest occupied`,"% and mean success rate was ",CurrentSummaryDF$`% nest success`,"%.",sep='')}
...
ggplot() +
  ...
  geom_point(data=subset(ProductivityDataFrame,Status=="Succeeded"), aes(x = Longitude, y = Latitude, colour=Status, size=factor(NumYng), na.rm = TRUE),color="black") +
...
# Occupancy rate %
MeanOccupancy = signif(round(mean(SuccessByYearDF$`% nest occupied`, na.rm = TRUE),1),2)
MinOccupancy= signif(min(SuccessByYearDF$`% nest occupied`, na.rm = TRUE),2)
MaxOccupancy= signif(max(SuccessByYearDF$`% nest occupied`, na.rm = TRUE),2)

# Success rate %
MeanSuccess = signif(mean(SuccessByYearDF$`% nest success`, na.rm = TRUE),2)
MinSuccess= signif(min(SuccessByYearDF$`% nest success`, na.rm = TRUE),2)
MaxSuccess= signif(max(SuccessByYearDF$`% nest success`, na.rm = TRUE),2)

# Productivity per nest
MeanYoungPerOccupiedNest = signif(mean(SuccessByYearDF$`Young/occupied nest`, na.rm = TRUE),3)
MinYoungPerOccupiedNest = signif(min(SuccessByYearDF$`Young/occupied nest`, na.rm = TRUE),3)
MaxYoungPerOccupiedNest = signif(max(SuccessByYearDF$`Young/occupied nest`, na.rm = TRUE),3)
MeanYoungPerSuccessfulNest = signif(mean(SuccessByYearDF$`Young/successful nest`, na.rm = TRUE),3)
MinYoungPerSuccessfulNest = signif(min(SuccessByYearDF$`Young/successful nest`, na.rm = TRUE),3)
MaxYoungPerSuccessfulNest = signif(max(SuccessByYearDF$`Young/successful nest`, na.rm = TRUE),3)
...
```{r echo=FALSE,label="Percent nest success by year",fig.alt=paste("Figure ",FigureCounter,". Nest success by year over the entire Copper River study area (",lmstats,"). Dashed line shows mean nest success rate of ",MeanSuccess,"% (full success and productivity metrics table is in Appendix).",sep=''),fig.cap=paste("Figure ",FigureCounter,". Nest success by year over the entire Copper River study area (",lmstats,"). Dashed line shows mean nest success rate of ",MeanSuccess,"% (full success and productivity metrics table is in Appendix).",sep='')}
...
lmstats = paste("Adj R2 = ",AdjR2,"Intercept = ",Intercept, " Slope =",Slope, " P =",P)

# plot % nest success
ggplot(data=SuccessByYearDF) +
  
  # Mean nest success line
  geom_line(mapping = aes(x=Year,y=MeanSuccess,),linetype = 2) +
...
```{r echo=FALSE,label="Young/successful nest",fig.alt=paste("Figure ",FigureCounter,". Young per successful nest by year over the entire Copper River study area (",lmstats,"). Mean young/successful nest [dashed line]=`r  MeanYoungPerSuccessfulNest`).  (full success and productivity metrics table is in Appendix).",sep=''),fig.cap=paste("Figure ",FigureCounter,". Young per successful nest by year over the entire Copper River study area (",lmstats,"). Mean young/successful nest [dashed line]=`r  MeanYoungPerSuccessfulNest`).  (full success and productivity metrics table is in Appendix).",sep='')}

# Make a linear model of young per successful nest by year
...
Bald eagle monitoring has been done from 1989 through `r MaxYear` (Appendix A). 

```{r echo=FALSE,label="Percent nest success by year by river segment",fig.alt=paste("Figure ",FigureCounter,". Nest success by year and river segment from , ",MinYear," - ",MaxYear," (full success and productivity metrics table is in Appendix).",sep=''),fig.cap=paste("Figure ",FigureCounter,". Nest success by year and river segment from , ",MinYear," - ",MaxYear," (full success and productivity metrics table is in Appendix).",sep='')}
...
Table `r TableCounter`. Long term success and productivity metrics by year and river segment in the Copper River bald eagles monitoring study area.
