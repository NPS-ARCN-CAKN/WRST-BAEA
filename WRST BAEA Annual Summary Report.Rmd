...
NF = CurrentSummaryDF$`Not found`
NC = CurrentSummaryDF$`Not counted`
YngPerOccupiedNest = CurrentSummaryDF$`Young/occupied nest`
YngPerNest = CurrentSummaryDF$`Young/successful nest`
PctOccupied = round((CurrentSummaryDF$Occupied - CurrentSummaryDF$`Unknown occupancy`) / CurrentSummaryDF$`Nests located` * 100,1)
PctSuccessful = round(CurrentSummaryDF$Successful / (CurrentSummaryDF$Occupied - CurrentSummaryDF$`Unknown productivity`) * 100,1)
...
Results for the `r SurveyYear` bald eagle survey in WRST appear in Table `r TableCounter`. Success and productivity metrics are reported in Table `r TableCounter + 1`. `r NestsLocated` nests were located during the survey. Of these nests, `r Occupied` were occupied (`r PctOccupied`%) as of the productivity survey. Of occupied nests `r Successful` produced young (`r PctSuccessful`% success rate, `r YngPerNest` mean young per nest).
...
, [Young/successful nest]
, [% nest success]
FROM            Summary_SuccessByYearAndRiverSegment
ORDER BY Year DESC")
...
CurrentSummaryDataFrame = SuccessByYearRiverSegmentDF[SuccessByYearRiverSegmentDF$Year == SurveyYear,2:11]
...
# Transpose the data
CurrentSummaryDataFrame_t = t(CurrentSummaryDataFrame)
colnames(CurrentSummaryDataFrame_t)[1:5] = "" # Change the wierd first column with no name that R stuck in to Result

# Output the data
kable(CurrentSummaryDataFrame_t)
...
ggplot() +
  ...
  # NOTE: Sometimes this layer doesn't convey high success very well
  # 2023 had high success in upper copper, but the kernel density didn't really
  # convey that.
  # stat_density_2d(
...
TribsSuccessDF = subset(SuccessByYearRiverSegmentDF,SuccessByYearRiverSegmentDF$`RiverSegment` != 'Chitina River Proper' & SuccessByYearRiverSegmentDF$`RiverSegment` != 'Chitina River Tributaries')
