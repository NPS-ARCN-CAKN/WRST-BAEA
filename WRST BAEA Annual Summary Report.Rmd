...
SurveyYear = 2023  # Set this to the desired report year. Don't set to 2016 because that survey didn't come off and the script will bomb
...
The 2016 survey was not finished due to bad weather and has been omitted from the results presented below.
...
Long-term bald eagle nest success and productivity metrics at WRST are shown in Figures 2 and 3. Results for the years 1989 through 1996 are from Kozie (1996).
...
FigureCaption = paste("Figure ",FigureCounter,". Nest success by year over the Copper River study area. Solid line shows mean nest occupancy rate (mean = ",MeanOccupancy,"%). Dashed line shows mean success rate for all occupied nests (mean = ",MeanSuccess,"%). Results for the years 1989 through 1996 are from Kozie (1996).",sep='')
...
# Set up the figure caption for the next figure
FigureCaption = paste("Figure ",FigureCounter,". Bald eagle productivity by year over the Copper River study area. Dashed line shows mean productivity for successful nests (occupied and productive) (mean = ",MeanYoungPerSuccessfulNest," young per nest). Dotted line shows mean productivity for all occupied nests (mean = ",MeanYoungPerOccupiedNest,"). Results for the years 1989 through 1996 are from Kozie (1996).",sep='')
```
...
# Quality Control Checks (Not to be published)

## Survey Attributes from the Surveys Database Table

### Survey details

```{r echo=FALSE}

df = GetDataFrameFromDatabase(paste("SELECT Year, Abstract, DataDirectory, ReportIRMACode, ReportURL, DataProcessingNotes, DatasetIRMAReferenceCode, DataQualityNotes
FROM Surveys
WHERE Year = ",SurveyYear,sep=''))
Abstract = df$Abstract
DataProcessingNotes = df$DataProcessingNotes
DataQualityNotes = df$DataQualityNotes

df2 = sqldf('SELECT DataDirectory, ReportIRMACode, ReportURL, DatasetIRMAReferenceCode FROM df')
kable(t(df2))
```
...
df = GetDataFrameFromDatabase("SELECT Nest_ID,Comments_O as Comment,'Occupancy' as Survey  FROM SurveyData WHERE Year = 2023 And (NOT (Comments_O IS NULL))
UNION
SELECT Nest_ID,Comments_P,'Productivity' FROM SurveyData WHERE Year = 2023 And (NOT (Comments_P IS NULL))
ORDER BY Nest_ID")
...
## Active nests with DateDiscoveredGone is not NULL

All active nests should have DateDiscoveredGone attributes NULL

```{r echo=FALSE}

df = GetDataFrameFromDatabase(paste("SELECT Nest_ID, Date_O, Nest_O, Date_P, StatusCode_P, Comments_O, Comments_P, IsActive, DateDiscoveredGone
...
## Nests With No Productivity Observation
...
df = GetDataFrameFromDatabase(paste("SELECT Nest_ID, RiverSegment, Date_O, OccupancyStatus, ProductivityStatus,Comments_O,Comments_p
...
## Successful Nests With No Recorded Young
...
df = GetDataFrameFromDatabase(paste("SELECT [Year]
      ,[Nest_ID]
      ,[NumYng]
      ,[StatusCode_P]
...
## Chick Age Recorded But No Young Recorded
...
df = GetDataFrameFromDatabase(paste("SELECT * FROM QC_ChickAgeButNoYoung WHERE Year = ",SurveyYear,sep=''))
kable(df)
