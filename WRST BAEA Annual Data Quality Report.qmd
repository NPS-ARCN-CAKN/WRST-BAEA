...
# Data Quality Report For The `r SurveyYear` Bald Eagles Survey Data Deliverable
...
High data quality is crucial for research on complex biological processes and ecosystems, allowing scientists to produce repeatable and reliable results, validate hypotheses, and assist in informed decision-making for the management of natural areas. The National Park Service recognizes data quality control as a critical step in the scientific life cycle for long-term monitoring of ecosystems and is committed to ensuring the accuracy, consistency, and completeness of data collected. 

This report documents the quality control procedures executed on the field data collected during the `r SurveyYear` bald eagles monitoring survey in Wrangell-St. Elias National Park and Preserve in Alaska. Quality control procedures are run on field data files both prior to, and after ingestion into the master database. Each section of this report details a quality control check performed on the master bald eagles monitoring database, along with any findings. Any identified quality control defects should be fixed or documented. The report is broken out into two major sections related to 1) information about eagle nests and 2) survey results.

# Quality Control Tests: Nests

## QC-N01: Nests With No Productivity Observation
...
```{r echo=FALSE}
Sql = paste0("SELECT [Year],[Nest_ID],[Nest_O],[StatusCode_P],[Comments_O],[Comments_p]
FROM [CAKN_BaldEagles].[dbo].[QC_SurveyData_MissingProductivity]
...
## QC-N02: Active nests that possibly should be inactive
...
## QC-N03: Minimum/Maximum Nest Spatial Coordinates
...
## QC-N04: Nests missing spatial coordinates  

The following nests are missing spatial coordinates. 

```{r echo=FALSE}  
Sql = paste("SELECT [Nest_ID],IsActive,DateDiscoveredGone,[Lon-NAD83],[Lat-NAD83],[Comments]  FROM [CAKN_BaldEagles].[dbo].[QC_Nests_MissingCoordinates] ORDER BY Nest_ID",sep="")
df = dbGetQuery(Connection,Sql)  
if(nrow(df)==0){  
    knitr::asis_output("*PASSED*. Zero records returned.  ")  
} else{  
...
## QC-N05: Active nests with DateDiscoveredGone is not NULL
...
## QC-N06: Nests with no observations

The following nests have no observations. They may have been lost the same survey year they were discovered. 

```{r echo=FALSE}  
Sql = paste("SELECT Nest_ID, DiscoverDate, TreeIsDead, DateDiscoveredGone, Comments FROM QC_Nests_NestsWithNoObservations ORDER BY DiscoverDate",sep="")
df = dbGetQuery(Connection,Sql)  
if(nrow(df)==0){  
    knitr::asis_output("*PASSED*. Zero records returned.  ")  
} else{  
...
The following nests, if any, should possibly be activated. Judgement is called for. Usually nests are judged as inactive if they were not found for two consecutive surveys. Some nests are noted as being visually observed to be gone or destroyed, and such nests will show up as false positives in this test.
...
FROM QC_SurveyData_ChickAgeButNoYoung WHERE Year = ",SurveyYear,sep='')
df = dbGetQuery(Connection,Sql)
if(nrow(df)==0){
  knitr::asis_output("*PASSED*. Zero records returned.  \n")
} else{
...
Tests = dbGetQuery(Connection,"SELECT[Table],[TableDescription],[SelectQuery] 
FROM [DatabaseTableDescriptions] WHERE 
[Table] NOT IN (
'QC_Nests_MinMaxBoundingBoxCheck'
)
And (Left([Table],3)='QC_') ORDER BY [Table]
")
i = 1
...
This template was generated automatically from an R Markdown script available at <https://github.com/NPS-ARCN-CAKN/WRST-BAEA-Annual-Report-R-Markdown>.
